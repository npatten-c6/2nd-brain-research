...
- **Repo-wide, not one file.** Tag every `docs/impl-research/` doc as `reference` or `lens` in `research-index.md` so the layer is legible across the whole set (e.g. `ob1-synthesis.md` = lens; `ob1-ingestion-recon.md` = reference; `nate-post-claims-audit.md` = lens; `nate-post-open-brain.md` = reference/source).
